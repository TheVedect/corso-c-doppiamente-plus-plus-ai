...
class Program {
   static void Main() {
        var context = new MLContext(seed: 2);
        ...
        var predictionEngine = context.Model.CreatePredictionEngine<InsuranceData, InsurancePrediction>(model);

        var marco = new InsuranceData { 
            Age = 40, 
            Gender = "male", 
            BMI = 28.5f, 
            Children = 2, 
        ...
        var prediction = predictionEngine.Predict(marco);
        Console.WriteLine($"Previsione costo per Marco: {prediction.PredictedCharges:0.##} $");
